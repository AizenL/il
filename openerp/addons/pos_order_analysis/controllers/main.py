...
class Home(http.Controller):
    ...
    def graph_data(self, redirect=None, **kw):
        ...
        if partner_id :
            cr.execute("""SELECT id from sale_order where date_order >= %s and date_order <= %s and state='done' and partner_id =%s""",(start_date,end_date,partner_id));
        else :
            cr.execute("""SELECT id from sale_order where date_order >= %s and date_order <= %s and state='done'""",(start_date,end_date));
        order_ids = cr.fetchall()
        pos_order_ids = [order_id[0] for order_id in order_ids]
        if product_id :
                product_query_op = ' in' if len(product_id) != 1 else  ' ='
                cr.execute("SELECT  DISTINCT(order_id) from sale_order_line where product_id "+ product_query_op + " %s",(tuple(product_id),))
        ...
        if pos_order_ids :
            ...
            if product_id :
                product_query_op = ' in' if len(product_id) != 1 else  ' ='
                cr.execute("SELECT  t.name_template,sum(product_uom_qty) from sale_order_line as p left join product_product t  on (p.product_id = t.id) where product_id " + product_query_op +"%s and order_id" + pie_query_op + "%s  group by t.name_template",(tuple(product_id),tuple(pos_order_ids),))
            else :
                cr.execute("SELECT  t.name_template,sum(product_uom_qty) from sale_order_line as p left join product_product t  on (p.product_id = t.id) where order_id" + pie_query_op + "%s  group by t.name_template",(tuple(pos_order_ids),))
            ...
            pos_order_obj = pool['sale.order']
            for pos_order in pos_order_obj.browse(cr,SUPERUSER_ID,pos_order_ids,context=context):
                if product_id :
                    product_query_op = ' in' if len(product_id) != 1 else  ' ='
                    cr.execute("SELECT  t.name_template,sum(product_uom_qty) from sale_order_line as p left join product_product t  on (p.product_id = t.id) where product_id" + product_query_op + "%s and order_id=%s group by t.name_template",(tuple(product_id),pos_order.id,))
                else :
                    cr.execute("""SELECT  t.name_template,sum(product_uom_qty) from sale_order_line as p left join product_product t  on (p.product_id = t.id) where order_id=%s group by t.name_template""",(pos_order.id,))
