...
class website_sale(http.Controller):
    ...
    def get_products(self, page=0, category=None, search='', **post):
        ...
        product_cat_obj = pool.get('product.category')
...
#        products = [{'label':x.name, 'category':x.categ_id.name} for x in product_obj.browse(cr, uid, product_ids, context=context)]
        ...
        cat_ids = set([x.categ_id.id for x in product_obj.browse(cr, uid, product_ids, context=context)])
        products = [{'category':x.id,'label': key_in_value + ' in '+ x.name} for  x in product_cat_obj.browse(cr,uid,cat_ids)]
#        pdb.set_trace()
        return products
