...
#            old_slipline_ids
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
        ...
        #SandV Code Shridhar - Start
        # pdb.set_trace()
        employee_id = payslip.employee_id.id
        from_date_formated = datetime.strptime(payslip.date_from,"%Y-%m-%d")
        to_date_formated = datetime.strptime(payslip.date_to,"%Y-%m-%d")
        variable_pay=self.pool.get('sandv.hr.payroll.variables')
        vaariable_employee_ids = variable_pay.search(cr,uid,[('name','=',employee_id),('end_date','<=',to_date_formated),('start_date','>=',from_date_formated)])
        
        variable_amount=variable_pay.browse(cr,uid,vaariable_employee_ids).variable_pay_rating
        other_allowance_amount=variable_pay.browse(cr,uid,vaariable_employee_ids).other_allowances
        #print vaariable_employee_ids
        #pdb.set_trace()

        # pdb.set_trace()
        
        salary_leave_days_deduction_obj = self.pool.get('hr.holidays')
        # salary_leave_days_deduction_brow = salary_leave_days_deduction_obj.browse(cr,uid,date_from, context=context)
        leave_type_obj = self.pool.get('hr.holidays.status')
        leave_type_id = leave_type_obj.search(cr,uid,[('name','=',"Unpaid")])
        # month_from_formated = datetime.strptime(salary_leave_days_deduction_brow,"%Y-%m-%d %H:%M:%S").month
        # salary_leave_days_deduction_ids = salary_leave_days_deduction_obj.search(cr,uid,[('employee_id','=',employee_id),('date_from','>=',date_from),('holiday_status_id','=',leave_type_id[0])])
        salary_leave_days_deduction_ids = salary_leave_days_deduction_obj.search(cr,uid,[('employee_id','=',employee_id),('holiday_status_id','=',leave_type_id[0])])
        payslip_month = datetime.strptime(payslip.date_from,"%Y-%m-%d").month
        payslip_year = datetime.strptime(payslip.date_from,"%Y-%m-%d").year
        leave_id_list = []
        for val in self.pool.get('hr.holidays').browse(cr, uid, salary_leave_days_deduction_ids, context=context):
            from_month = datetime.strptime(val.date_from,"%Y-%m-%d %H:%M:%S").month
            to_month = datetime.strptime(val.date_to,"%Y-%m-%d %H:%M:%S").month
            from_year = datetime.strptime(val.date_from,"%Y-%m-%d %H:%M:%S").year
            to_year = datetime.strptime(val.date_from,"%Y-%m-%d %H:%M:%S").year

            if from_month == payslip_month and from_year == payslip_year:
                leave_id_list.append(val.id)
            if to_month == payslip_month and to_year == payslip_year:
                leave_id_list.append(val.id)
        leave_id_list = list(set(leave_id_list))
        month_no_of_days = (to_date_formated - from_date_formated).days + 1
        # month_no_of_days = 30.00
        baselocaldict.update({'month_no_of_days':month_no_of_days})
        # print "\n\n\n\n\nn\++++",month_no_of_days
        # print "\n\n\n\nn\+++++payslip month",leave_id_list,payslip_month

        deduction_days = 0
        same_month_days = 0
        different_month_days = 0
        no_of_unpaid_leaves=0

        baselocaldict.update({'no_of_unpaid_leaves':no_of_unpaid_leaves,'month_no_of_days':month_no_of_days,'variable_pay':variable_amount,})
        # pdb.set_trace()
        if leave_id_list :
           
            for obj in self.pool.get('hr.holidays').browse(cr, uid, leave_id_list, context=context):
                month_from_formated = datetime.strptime(obj.date_from,"%Y-%m-%d %H:%M:%S").month
                month_to_formated = datetime.strptime(obj.date_to,"%Y-%m-%d %H:%M:%S").month
                # print "\n\n\n\n\n\nn\n\n\n++++++",month_from_formated,month_to_formated
                if month_from_formated != month_to_formated:
                    day_from_formated = datetime.strptime(obj.date_from,"%Y-%m-%d %H:%M:%S").day
                    year_from_formated = datetime.strptime(obj.date_from,"%Y-%m-%d %H:%M:%S").year
                    month_from_formated = datetime.strptime(obj.date_from,"%Y-%m-%d %H:%M:%S").month
                    from_last_date = calendar.monthrange(year_from_formated,month_from_formated)[1] - day_from_formated + 1
                    # print "\n\n\n\n\n++++",day_from_formated
                    # print "\n\n\n\nn\n++++",from_last_date
                    day_to_formated = datetime.strptime(obj.date_to,"%Y-%m-%d %H:%M:%S").day
                    year_to_formated = datetime.strptime(obj.date_to,"%Y-%m-%d %H:%M:%S").year
                    month_to_formated = datetime.strptime(obj.date_to,"%Y-%m-%d %H:%M:%S").month
                    to_last_date = day_to_formated
                    # print "\n\n\n\n\n++++",day_to_formated
                    # print "\n\n\n\nn\n++++",to_last_date
                    if payslip_month == month_from_formated:
                        different_month_days = from_last_date
                    if payslip_month == month_to_formated:
                        different_month_days = to_last_date
                    # print "\n\n\nn\n\n\n++++dif ",different_month_days
                if month_from_formated == month_to_formated:
                    same_month_days += obj.number_of_days_temp
            # print "\n\n\n\n\nn\n++++++days",different_month_days,same_month_days
            no_of_unpaid_leaves = different_month_days+same_month_days
            # print "\nn\n\n\n\nn\n++++",basic,month_no_of_days,deduction_days,(basic/month_no_of_days)*deduction_days
#            deduction_amount= deduction_days
#            unpaid_leave_deduction_obj = unpaid_leave_deduction_obj + deduction_amount



        
        baselocaldict.update({'no_of_unpaid_leaves':no_of_unpaid_leaves})
        #SandV Shridhar Ends
        

#SANDV updates -----Starts
        employee_id = payslip.employee_id.id
        ...
        baselocaldict.update({'tds' : tds, 'leave_deductions' : leave_deductions, 'advance_deductions' : advance_deductions, 'other_deductions'  : other_deductions})
        ...
        salary_variable_amount_obj = self.pool.get('sandv.hr.payroll.variables')
        salary_variable_amount_ids = salary_variable_amount_obj.search(cr,uid,[('name','=',employee_id),('start_date','>=',date_from),('end_date','<=',date_to)])
     
        # pdb.set_trace()
        employee_obj = self.pool.get('hr.employee')
        emp_date_from = datetime.strptime(payslip.date_from,"%Y-%m-%d")
        emp_date_to = datetime.strptime(payslip.date_to,"%Y-%m-%d")

        vaariable_employee_ids = variable_pay.search(cr,uid,[('name','=',employee_id),('end_date','<=',emp_date_to),('start_date','>=',emp_date_from)])
        #pdb.set_trace
        variable_amount=variable_pay.browse(cr,uid,vaariable_employee_ids).variable_pay_rating
        other_allowance_amount = variable_pay.browse(cr,uid,vaariable_employee_ids).other_allowances
        other_deductions_amount = variable_pay.browse(cr,uid,vaariable_employee_ids).other_deductions

        baselocaldict.update({'variable_amount' : variable_amount,'other_allowance_amount':other_allowance_amount,'other_deductions_amount':other_deductions_amount})
        # sal_variable_pay = employee_obj.variable_pay_rating
        # sal_other_allow = employee_obj.other_allowances
        # sal_other_ded = employee_obj.other_deductions
        # emp_start_date = employee_obj.search(cr,uid,[('name','=',employee_id),('start_date','>=',date_from),('end_date','<=',date_to)])
        # if salary_variable_amount_ids : 
        #     for salary_variable_amount_line in self.pool.get('sandv.hr.payroll.variables').browse(cr, uid, salary_variable_amount_ids, context=context):
        #         variable_amount = variable_amount + salary_variable_amount_line.variable_amount
        #         leave_deductions = leave_deductions + salary_variable_amount_line.leave_deductions
        #         advance_deductions = advance_deductions + salary_variable_amount_line.advance_deductions
        #         other_deductions =other_deductions + salary_variable_amount_line.other_deductions
        #         baselocaldict.update({'variable_amount' : variable_amount,})
