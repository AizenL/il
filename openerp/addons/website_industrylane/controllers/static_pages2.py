...
#     def get_ChildCategoryList(self,more_than_five_ids):
#         cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
#         category_obj = pool['product.public.category']
#         more_than_five_ids_brow = category_obj.browse(cr, uid, more_than_five_ids, context=context)
#         category_id1 = category_obj.search(cr, uid, [('parent_id', '=', more_than_five_ids)], context=context)
#         category_id1_brow = category_obj.browse(cr, uid, category_id1, context=context)
#         values = {
#             'ChildIds' : category_id1_brow,
#             'ParentIds':more_than_five_ids_brow,
#                   }
#         return values

    def get_parentproductlist(self,more_than_five_ids):
        cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        category_obj = pool['product.public.category']
        more_than_five_ids_brow = category_obj.browse(cr, uid, more_than_five_ids, context=context)
        return more_than_five_ids_brow    
    ...
    def shop_category(self, page=0, category=None, search='', **post):
        cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        ...
        #nonemptycategs = category_obj.search(cr, uid, [('parent_id', '=', False)], context=context)
        ...
        return request.website.render("website_industrylane.shop_category", values)


#  
    def OperationSoma(self,products,ppg,ppr):
        global PPR
        PPR = ppr
        #pdb.set_trace()
    ...
    def shop(self, page=0, category=None, search='', **post):
        ...
        top_categories_ids = category_obj.search(cr,uid,[('parent_id', '=', False)], limit=6,context=context)
        top_categories = category_obj.browse(cr, uid, top_categories_ids, context=context)
        
        #category_ids_active = category_obj.search(cr, uid, [('active_products', '=', True)], context=context)
                
        #nonemptycategs = category_obj.search(cr, uid, [('parent_id', '=', False)], context=context)
        #featuredProductsids =  product_obj.search(cr, uid, [('website_style_ids', '=', 'Featured')], context=context)
        #featuredpager = request.website.pager(url=url, total=product_count, page=page, step=10, scope=7, url_args=post)
        featuredProductsids =  product_obj.search(cr, uid, [('website_style_ids', '=', 'Featured')], limit=10, offset=pager['offset'], order=self._get_search_order(post), context=context)
        FeaturedProducts = product_obj.browse(cr, uid, featuredProductsids, context=context)
        ...
        values = {
            ...
            'pricelist': pricelist,
            'products': products,
            #'bins': table_compute().process(products,20,4),
            'bins': self.OperationSoma(products,20,4),            
            'rows': PPR,
            'styles': styles,
            'categories': categs,
            'featured': self.OperationSoma(FeaturedProducts,20,10),
            'top_categories': top_categories,                          
            #'featured': table_compute().process(FeaturedProducts,20,10),            
            'attributes': attributes,
            #'category_ids_active':category_ids_active,
            'compute_currency': compute_currency,
            'keep': keep,
            'style_in_product': lambda style, product: style.id in [s.id for s in product.website_style_ids],
            'attrib_encode': lambda attribs: werkzeug.url_encode([('attrib',i) for i in attribs]),
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        #return request.website.render("website.layout", values)
    ...
    def cart(self,category='', search='', **post):
        ...
        return request.website.render("website_industrylane.custom_cart", values)
    ...
    def cart_update_json(self, product_id, line_id, add_qty=None, set_qty=None, display=True):
        ...
        value['website_industrylane.total'] = request.website._render("website_industrylane.total", {
                'website_industrylane_order': request.website.sale_get_order()
            })
        return value
    ...
    def checkout_values(self, data=None):
        ...
        category_obj = registry.get('product.public.category')
        #category_obj = pool['product.public.category']
