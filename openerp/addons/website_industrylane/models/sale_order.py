...
class sale_order(osv.Model):
    ...
    _columns = {
        'website_order_line': fields.one2many(
            'sale.quotation.order.line', 'order_id',
    ...
    def _cart_find_product_line(self, cr, uid, ids, product_id=None, line_id=None, context=None, **kwargs):
        for so in self.browse(cr, uid, ids, context=context):
            ...
            if line_id:
                domain += [('id', '=', line_id)]
            return self.pool.get('sale.quotation.order.line').search(cr, SUPERUSER_ID, domain, context=context)

    def _website_product_id_change(self, cr, uid, ids, order_id, product_id, qty=0, line_id=None, context=None):
        so = self.pool.get('sale.quotation').browse(cr, uid, order_id, context=context)

        values = self.pool.get('sale.quotation.order.line').product_id_change(cr, SUPERUSER_ID, [],
        ...
        if line_id:
            line = self.pool.get('sale.quotation.order.line').browse(cr, SUPERUSER_ID, line_id, context=context)
...
#             values['tax_id'] = [(6, 0, values['tax_id'])]
    ...
    def _cart_update(self, cr, uid, ids, product_id=None, line_id=None, add_qty=0, set_qty=0, context=None, **kwargs):
        ...
        sol = self.pool.get('sale.quotation.order.line')
...
#                     add_qty -= 1
            ...
            product = self.pool.get('product.product').browse(cr, uid, product_id, context=context)        
            product_quantity_type = product.product_tmpl_id.type_quantity1
            product_minimum_qty =  product.product_tmpl_id.min_quantity1             
#             pdb.set_trace()
            if add_qty:                                
                if product_quantity_type =='Multiple':
                    if not(add_qty%product_minimum_qty == 0):
                        quantity = product_minimum_qty
                    else :
                        quantity = add_qty
                if product_quantity_type =='Minimum':
                    if add_qty<product_minimum_qty:
                        quantity = product_minimum_qty
                    else :
                        quantity = add_qty      
            ...
            if set_qty:
                if product_quantity_type =='Multiple':
#                     pdb.set_trace()
                    if not(set_qty%product_minimum_qty == 0):
                        quantity = product_minimum_qty
                    else :
                        quantity = set_qty
                if product_quantity_type =='Minimum':
                    if set_qty<product_minimum_qty:
                        quantity = product_minimum_qty
                    else :
                        quantity = set_qty                         
            # compute new quantity
            if add_qty == 0 and set_qty ==0 and quantity == 0 : 
...
#             elif add_qty != None:
#                 pdb.set_trace()
#                 quantity = sol.browse(cr, SUPERUSER_ID, line_id, context=context).product_uom_qty
#                 quantity = add_qty
#             elif add_qty == 0:
#                 pdb.set_trace()
#                 quantity = sol.browse(cr, SUPERUSER_ID, line_id, context=context).product_uom_qty + (add_qty or 0)
...
class sale_order_line(osv.Model):
    _inherit = "sale.quotation.order.line"
...
class website(orm.Model):
    ...
    def sale_get_order(self, cr, uid, ids, force_create=False, code=None, update_pricelist=None, context=None):
        ...
        sale_order_obj = self.pool['sale.quotation']
        partner =  self.pool['res.users'].browse(cr, SUPERUSER_ID, uid, context=context).partner_id
        get_sale_order_id = sale_order_obj.search(cr, SUPERUSER_ID, [('partner_id', '=', partner.id),('state', 'in', ['draft'])], context=context)
        sale_order_id = get_sale_order_id and max(get_sale_order_id)
