            ...
            if tx and tx.sale_order_id:
                ...
                if amount_matches:
                    if tx.state == 'done':
                        ...
                        self.pool['sale.quotation'].action_button_confirm(cr, SUPERUSER_ID, [tx.sale_order_id.id], context=dict(context, send_email=True))
                        ...
                        self.pool['sale.quotation'].force_quotation_send(cr, SUPERUSER_ID, [tx.sale_order_id.id], context=context)
                else:
