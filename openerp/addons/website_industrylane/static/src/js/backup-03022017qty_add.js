...
$('.oe_website_sale').each(function () {
 ...
 // hack to add and remove from cart with json
    ...
    $($oe_website_sale).on('click', 'a.js_add_cart_json_inherited', function (ev) {
		...
		//var min_order_qty = document.getElementById("cart_quantity_txt_box").getAttribute('data-minmum');
		//var order_type = document.getElementById("cart_quantity_txt_box").getAttribute('data-ordertype');
		var min_order_qty = parseFloat($input.data('minmum'));
		var order_type = $input.data('ordertype');
		
		var $input_value_qty = parseFloat($input.val(),10);
		
		console.log($input_value_qty+"---"+min_order_qty);
		...
		if (order_type == 'Multiple'){			
			...
			if (min_order_qty >$input_value_qty){
				console.log("Less them Minimum Order Qty Not Allow");
			}
        ...
        $input.val(quantity > min_order_qty ? (quantity < max ? quantity : max) : min_order_qty);
				$('input[name="'+$p_id+'"]').val(quantity > min_order_qty ? (quantity < max ? quantity : max) : min_order_qty);
				$input.change();
        return false;
    });
