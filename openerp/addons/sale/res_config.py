...
Example: 10% for retailers, promotion of 5 EUR on this product, etc."""),
        ...
        'cst_vat':fields.float('CST/VAT %',store="True",default_model='sandv.customer.pricelist'),
        'entry_tax':fields.float('Entery Tax %',store="True",default_model='sandv.customer.pricelist'),
    }
    def get_default_values(self, cr, uid, ids, context=None):
        # pdb.set_trace()
        param_obj = self.pool.get("ir.config_parameter")
        cst_vat = param_obj.get_param(cr, uid, "cst_vat")
        new_extra = param_obj.get_param(cr, uid, "entry_tax")
        #print "cat",int(cst_vat)
        cst = float(cst_vat or 0.0)
        final_extra1 = float(new_extra or 0.0)
        res = {'cst_vat':cst ,'entry_tax':final_extra1
            }
        return res
    ...
    def set_values(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            param_obj.set_param(cr, uid, "cst_vat", record.cst_vat or '')
            param_obj.set_param(cr, uid, "entry_tax", record.entry_tax or '') 
